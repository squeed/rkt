...
func (e *podEnv) setupNets(nets []activeNet, noDNS bool) error {
	...
	for i, n = range nets {
		if debuglog {
			stderr.Printf("loading network %v with type %v", n.conf.Name, n.conf.Plugins[0].Network.Type)
		}
		...
		if !common.IsDNSZero(&n.runtime.CniResult.DNS) && !noDNS {
			if !podHasResolvConf {
				err := ioutil.WriteFile(
					resolvPath,
					[]byte(common.MakeResolvConf(n.runtime.CniResult.DNS, "Generated by rkt from network "+n.conf.Name)),
					0644)
				if err != nil {
...
func (e *podEnv) teardownNets(nets []activeNet) {
	...
	for i := len(nets) - 1; i >= 0; i-- {
		if debuglog {
			stderr.Printf("teardown - executing net-plugin %v", nets[i].conf.Plugins[0].Network.Type)
		}
...
func loadNet(filepath string) (*activeNet, error) {
	var cl *libcni.NetworkConfigList
	var err error
	if strings.HasSuffix(filepath, ".conflist") {
		cl, err = libcni.ConfListFromFile(filepath)
		if err != nil {
			return nil, err
		}
	} else if strings.HasSuffix(filepath, ".conf") || strings.HasSuffix(filepath, ".json") {
		c, err := libcni.ConfFromFile(filepath)
		if err != nil {
			return nil, err
		}

		cl, err = libcni.ConfListFromConf(c)
		if err != nil {
			return nil, err
		}
	} else {
...
func loadUserNets(localConfig string, netsLoadList common.NetList) ([]activeNet, error) {
	...
	files, err := libcni.ConfFiles(userNetPath, []string{".conflist", ".conf", ".json"})
	if err != nil {
		return nil, err
	}
	sort.Strings(files)
	...
	for _, file := range files {
		...
		if n.conf.Name == "default" ||
			...
			stderr.Printf(`overriding %q network with %v`, n.conf.Name, filepath.Base(file))
		}

		if netExists(nets, n.conf.Name) {
			stderr.Printf("%q network already defined, ignoring %v", n.conf.Name, filepath.Base(file))
			continue
		}
