...
func parseFlags() *stage1commontypes.RuntimePod {
	...
	flag.BoolVar(&rp.InsecureOptions.Capabilities, "disable-capabilities-restriction", false, "Disable capability restrictions")
	flag.BoolVar(&rp.InsecureOptions.Paths, "disable-paths", false, "Disable paths restrictions")
	flag.BoolVar(&rp.InsecureOptions.Seccomp, "disable-seccomp", false, "Disable seccomp restrictions")
...
func mountContainerV1Cgroups(m fs.Mounter, p *stage1commontypes.Pod, enabledCgroups map[int][]string, subcgroup string, serviceNames []string) error {
	...
	if err := v1.RemountCgroups(m, stage1Root, enabledCgroups, subcgroup, serviceNames, p.InsecureOptions.Paths); err != nil {
