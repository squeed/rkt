...
func mountSharedVolumes(root string, p *stage1commontypes.Pod, ra *schema.RuntimeApp) error {
	...
	mounts, err := stage1initcommon.GenerateMounts(ra, p.Manifest.Volumes, imageManifest)
	if err != nil {
		return err
	}
	for _, m := range mounts {
		...
		} else if err := doBindMount(cleanedSource, absDestination, m.ReadOnly, vol.Recursive); err != nil {
